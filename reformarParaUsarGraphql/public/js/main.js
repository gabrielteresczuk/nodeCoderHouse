    ...
    fetch('http://localhost:8080/productogql', {
      ...
      .then(data => {
          ...
          if(data.data.getProductos){
              ...
              html+=data.data.getProductos.map(el => {
                  return (`<tr>
                  <td>${el.nombre}</td>
                  <td>${el.precio}</td>
              ...
              html+=`</tbody><tfoot><tr><td colspan="4"># Productos: ${data.data.getProductos.length}</td></tr></tfoot></table>`;
...
.then(data => {
    ...
    `;
    if(data.data.getProductosTest){
        html+=`<table><thead><th>Nombre</th><th>Precio</th><th>Imagen</th></thead><tbody>`;

        html+=data.data.getProductosTest.map(el => {
            return (`<tr>
            <td>${el.nombre}</td>
            <td>${el.precio}</td>
            <td><img src=${el.foto} alt="imagen" /></td>
        ...
        html+=`</tbody><tfoot><tr><td colspan="3"># Productos: ${data.data.getProductosTest.length}</td></tr></tfoot></table>`;
    }else{
        html +=`<p class="nohay">No hay productos cargados</p>`;
    }
